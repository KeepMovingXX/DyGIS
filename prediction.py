...
from utils import mask_edges_det, load_dataset, mask_edges_prd, mask_edges_prd_new
...
for repeat in range(args.run_times):
    ...
    edge_all_list, edge_droped_idx_list, edge_idx_list = generate_is_subgraph(args, x_in_list[0], edge_list,
                                                                           adj_orig_dense_list, num_nodes)
    ...
    for k in range(args.epochs_task):
        ...
        if k > args.test_after:
            model.eval()
            _, _, _, enc_means, pri_means, _, _, _ = model(x_in[(seq_len - args.spilt_len):seq_len]
                                                         , edge_all_list[(seq_len - args.spilt_len):seq_len]
                                                         , edge_all_list[(seq_len - args.spilt_len):seq_len]
                                                         , edge_droped_idx_list[(seq_len - args.spilt_len):seq_len]
                                                         , hidden_st, pridiction=True)

            auc_scores_det_val, ap_scores_det_val = get_roc_score(pos_edges_l[(seq_len - args.spilt_len):seq_len]
                                                                  , false_edges_l[(seq_len - args.spilt_len):seq_len]
                                                                  , pri_means)

            auc_scores_det_test, ap_scores_det_tes = get_roc_score(pos_edges_l_n[(seq_len - args.spilt_len):seq_len]
                                                                   , false_edges_l_n[(seq_len - args.spilt_len):seq_len]
                                                                   , pri_means)
            ...
            if auc_scores_det_val > best_auc_val and ap_scores_det_val > best_ap_val:
                ...
                print('pri_link_auc_mean', best_auc_val)
                print('pri_link_ap_mean', best_ap_val)
                print('pri_new_link_auc_mean', best_auc_test)
                print('pri_new_link_ap_mean', best_ap_test)
                print('----------------------------------')
            else:
                patience += 1
        print('epoch: ', k)
    ...
    print('Link Prediction')
    print('prediction_auc_mean', best_auc_val)
    print('prediction_ap_mean', best_ap_val)
    print('prediction_new_auc_mean', best_auc_test)
    print('prediction_new_ap_mean', best_ap_test)
    print('----------------------------------')
...
with open(log_fname, 'a') as f:
    f.write(
        '\n'.join(
            (f'-----Run for Link prediction times: {args.run_times}',
             'prediction metrics muitiple times:',
             f'ROC-AUCs, AP-AUCs: {val_auc_mean_value, val_auc_mean_std, val_ap_mean_value, val_ap_mean_std}',
             f'new prediction metrics muitiple times:',
